...
export const EffectsPanel: React.FC<EffectsPanelProps> = ({ colorScheme, setColorScheme, filters, setFilterValue, onReset }) => (
    <div className="p-4 bg-gray-800 rounded-lg border border-gray-700 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                <label htmlFor="sate" className="block text-sm font-medium text-gray-400 mb-2">Saturation: <span className="font-bold text-white">{filters.saturate}%</span></label>
                <input type="range" id="sate" min="0" max="200" value={filters.saturate} onChange={e => setFilterValue('saturate', parseInt(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-pink-500" />
            </div>
        </div>
    </div>
);
